...
CSF::createSection($prefix, array(
    ...
    'fields' => array(
        ...
        array(
            'id' => 'g_cdn_rewrite',
            'type' => 'switcher',
            'title' => __('自定义 CDN 域名重写', 'kratos'),
            'subtitle' => __('按文件后缀把站内静态资源的域名替换为指定 CDN 域名', 'kratos'),
            'default' => false,
        ),
        array(
        ...
        array(
            'id' => 'g_cdn_rewrite_mode',
            'type' => 'select',
            'title' => __('后缀匹配模式', 'kratos'),
            'subtitle' => __('include 只替换列表内后缀；exclude 除列表内以外全部替换', 'kratos'),
            'options' => array(
                'include' => __('包含指定后缀（仅替换列表内）', 'kratos'),
                'exclude' => __('排除指定后缀（列表外全部替换）', 'kratos'),
            ),
            'default' => 'include',
            'dependency' => array('g_cdn_rewrite', '==', 'true'),
        ),
        array(
            'id' => 'g_cdn_rewrite_exts',
            'type' => 'textarea',
            'title' => __('文件后缀列表', 'kratos'),
            'subtitle' => __('用逗号、空格或换行分隔；不带点，如 js, css, png, jpg, webp, woff2', 'kratos'),
            'default' => 'js,css,png,jpg,jpeg,gif,webp,svg,ico,woff,woff2,ttf,otf,eot,mp4,webm,mp3',
            'dependency' => array('g_cdn_rewrite', '==', 'true'),
        ),
        array(
            'id' => 'g_animate',
...
CSF::createSection($prefix, array(
    ...
    'fields' => array(
        ...
        array(
            'type' => 'subheading',
            'content' => __('嵌套折叠', 'kratos'),
        ),
        array(
            'id' => 'g_comment_flatten_enabled',
            'type' => 'switcher',
            'title' => __('深层回复折叠', 'kratos'),
            'subtitle' => __('顶层评论下的回复超过指定条数后，多余回复平铺到「加载更多回复」按钮下按需加载；回复按钮始终显示', 'kratos'),
            'text_on' => __('开启', 'kratos'),
            'text_off' => __('关闭', 'kratos'),
            'default' => false,
        ),
        array(
            'id' => 'g_comment_flatten_depth',
            'type' => 'number',
            'title' => __('折叠起始序号', 'kratos'),
            'subtitle' => __('顶层评论下按 DFS 顺序保留前 N 条回复直接展示，第 N+1 条起折叠', 'kratos'),
            'default' => 3,
            'min' => 1,
            'max' => 50,
            'dependency' => array('g_comment_flatten_enabled', '==', 'true'),
        ),
        array(
            'id' => 'g_comment_flatten_page_size',
            'type' => 'number',
            'title' => __('每次加载条数', 'kratos'),
            'subtitle' => __('点击「加载更多回复」时每次拉取的深层回复条数', 'kratos'),
            'default' => 5,
            'min' => 1,
            'max' => 50,
            'dependency' => array('g_comment_flatten_enabled', '==', 'true'),
