...
/**
 * 获取1:1还原的精美SVG图标（通过$GLOBALS['UA_ICON']调用，Unknown兜底）
 * @param string $type 类型：browser/os
 * @param string $name 名称（如Chrome/Windows 10）
 * @return string 精准还原的SVG图标HTML
 */
function wpcdi_get_svg_icon($type, $name) {
    // 基础尺寸（14x14，viewBox精准适配）
    $size = 14;
    $icon_key = '';
    ...
    if ($type === 'browser') {
        ...
        if (strpos($name, 'Windows') !== false) {
            ...
            $icon_key = 'Android';
        } elseif (strpos($name, 'iPhone') !== false || strpos($name, 'iPad') !== false || strpos($name, 'iPod') !== false || strpos($name, 'iOS') !== false) {
            $icon_key = 'iOS';
        } elseif (strpos($name, '鸿蒙系统') !== false || strpos($name, 'HarmonyOS') !== false) {
            $icon_key = 'HarmonyOS';
        } elseif (strpos($name, 'Linux') !== false || strpos($name, 'Chrome OS') !== false) {
            $icon_key = 'Linux';
        } elseif (strpos($name, 'BlackBerry') !== false) {
            $icon_key = 'BlackBerry';
        } elseif (strpos($name, 'Kindle Fire') !== false) {
            $icon_key = 'KindleFire';
        } else {
            $icon_key = 'Unknown';
        }
    ...
    // 核心调用：优先取对应图标，未知则用Unknown
    $svg = $GLOBALS['UA_ICON'][$icon_key] ?? $GLOBALS['UA_ICON']['Unknown'];

    // 统一添加样式（保持原插件的垂直对齐/间距）
    $svg = str_replace(
        '<svg',
        '<svg width="'.$size.'" height="'.$size.'" style="vertical-align:middle;margin-right:2px;"',
        $svg
    );
